...
class Jogador {
    ...
    constructor(forca: number, nivel: number, pontosDeVida: number) {
        this.forca = forca;
        this.nivel = nivel;
...
async function showBattleMessage(text: String, nome: String, jogador: Jogador, inimigo?: Jogador) {
    showBattleInterface(nome, jogador, inimigo);
    skipLines(1);
    console.log(`>> ${text}`);
    enterToContinue();
...
function showBattleInterface(nome: String, jogador: Jogador, inimigo?: Jogador) {
    limparTerminal();
    showCenterText(`${nome} - ${jogador.toString()}`)
    jogador.showHP();
    if (jogador.experiencia > 0) showCenterText(jogador.experiencia + " exp.");
    else skipLines(1);
    if (inimigo == undefined) return;
    skipLines(1);
    showCenterText(`Inimigo - ${inimigo.toString()}`);
    inimigo.showHP();
}

function skipLines(_n: number) {
    for (let i = 0; i < _n; i++) {
        console.log();
    }
}
...
function showCenterText(_txt: string) {
...
function mostrar_mensagem(_mensagem: string) {
    let msg = `\nPATROBOT: ${_mensagem}`;
    let intervalo = 2;
    return new Promise(resolve => {
        let i = 0;
